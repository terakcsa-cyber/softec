...
function extractVersionRanges(raw: Record<string, unknown>): string[] {
  ...
  } else if (asRecord(configs)?.nodes && Array.isArray((configs as Record<string, unknown>).nodes)) {
    nodes.push(...((configs as Record<string, unknown>).nodes as unknown[]));
  }
...
function extractReferenceFixes(raw: Record<string, unknown>): {
  ...
  for (const item of refs) {
    ...
    if (/security\..debian\.org|lists\.ubuntu\.com|access\.redhat\.com/i.test(url)) {
      mitigation.push(`Проверить бюллетень дистрибутива: ${url}`);
    }
  }
...
/** Извлекает actionable fix/remediation из NVD CVE 2.0 JSON (configurations + references). */
export function extractFixSignalsFromNvdRaw(raw: unknown): NvdFixSignals {
  ...
  if (remediation.length === 0 && patchLinks.length === 0 && versionLines.length === 0) {
    nextSteps.push("Проверить advisories вендоров и ссылки NVD на наличие патчей");
  }
...
/** Дополняет output_json ИИ remediation/nextSteps из NVD, если модель их не заполнила. */
